...
def get_frame(features: list, player_table) -> pd.DataFrame:
    """Function to convert scraped data into pandas dataframe.

    Args:
        features (list): List of columns to keep.
        player_table (_type_): Table containing scraped data.

    Returns:
        pd.DataFrame: Dataframe containing scraped data and columns desired.
    """
    pre_df_player = dict()
    ...
    for row in rows_player:
        if(row.find('th',{"scope":"row"}) != None):    
            for f in features_wanted_player:
                ...
                text=a.decode("utf-8")
                if(text == ''):
                    # TODO: Replace this with an np.nan
                    # text = '0'
                    text = np.nan
                # Convert non-string columns to floats
                if (not f in ['player', 'nationality', 'position', 'team', 'age', 'birth_year']) & (not type(text) == float):
    ...
    # Convert dictionary to dataframe for output
...
def frame_for_category_team(category,top,end,features,text):
    url = (top + category + end)
    _, team_table = get_tables(url,text)
    df_team = get_frame_team(features, team_table)
    return df_team


def get_outfield_data(top: str, end: str, stats: list) -> pd.DataFrame:
    ...
    Args:
        top (str): Website URL path (top)
        end (str): Website URL path (end)
